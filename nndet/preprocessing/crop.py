...
从文件中加载图像和分割标签数据。
裁剪图像和分割标签，确保裁剪后的数据仅包含非零区域（通常背景像素为零）。
对图像数据进行频域转换。
保存处理后的数据到 .npz 和 .pkl 文件
"""
...
def create_nonzero_mask(data: np.ndarray) -> np.ndarray:#它对输入数据的每个通道（C 维度）进行检查，找出所有非零的像素点，并生成一个掩码
    """
...
#接受的是上一步生成的非零的像素点的掩码？输出该 bounding box 的坐标。 bounding box就是上一步的掩码生成的框
...
def crop_to_bbox_no_channels(image, bbox: Sequence[Sequence[int]]):#作用2D
    """
...
def crop_to_bbox(data: np.ndarray, bbox: Sequence[Sequence[int]]):#作用3D数据，4维，
    """
...
def crop_to_nonzero(data, seg=None, nonzero_label=-1):
    ...
    Returns:
        ...
        对框进行裁剪，确保裁剪后的图像和分割标签仅包含非零像素的部分
        在裁剪标签seg时使用的是与图像数据相同的 bounding box。因此图像数据和标签会被裁剪到相同的区域
    """
...
#主控文件class，包含了两个主要功能：裁剪图像和标签 以及 频域转换。
class ImageCropper(object):
    ...
    @staticmethod
    def load_crop_from_list_of_files(data_files: List[Path], seg_file: Path = None):#这个方法加载数据文件和分割标签文件，并调用 crop 函数来进行裁剪。
        """
