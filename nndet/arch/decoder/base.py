...
class CombinedDecoder(nn.Module):
    def __init__(self,
                 ...
                 norm_fusion: bool = False,
                 activation_fusion: bool = False,
                 nnformer_params: dict = None):
        """
        将 RetinaUnet 的解码器和 nnFormer 的解码器整合到一起。
        """
        super().__init__()

        # 初始化 UFPNModular
        self.ufpn = UFPNModular(conv=conv,
                                strides=strides,
        ...
        # 初始化 nnFormer 解码器
        self.nnformer_decoder = nnFormerDecoder(**nnformer_params)

        # 定义一个融合层，用于融合 UFPN 和 nnFormer 的特征
        self.fusion_layers = nn.ModuleList()
        ufpn_channels = self.ufpn.get_channels()
        nnformer_channels = self.nnformer_decoder.num_features

        for u_channel, n_channel in zip(ufpn_channels, nnformer_channels):
            fusion_conv = conv(u_channel + n_channel,
                               fixed_out_channels,
                               kernel_size=3,
                               padding=1,
                               stride=1,
                               norm=True,
                               activation=True)
            self.fusion_layers.append(fusion_conv)

    def forward(self, inp_seq: Sequence[torch.Tensor], encoder_skips: Sequence[torch.Tensor]) -> List[torch.Tensor]:
        # 使用 UFPN 进行解码
        ufpn_features = self.ufpn(inp_seq)

        # 使用 nnFormer 解码器进行解码
        nnformer_features = self.nnformer_decoder(inp_seq[-1], encoder_skips)

        # 融合特征
        combined_features = []
        for idx, (u_feat, n_feat, fusion_layer) in enumerate(zip(ufpn_features, nnformer_features, self.fusion_layers)):
            # 确保尺寸匹配
            if u_feat.shape != n_feat.shape:
                n_feat = nn.functional.interpolate(n_feat, size=u_feat.shape[2:], mode='trilinear', align_corners=False)

            # 拼接并通过融合层
            combined_feat = torch.cat([u_feat, n_feat], dim=1)
            combined_feat = fusion_layer(combined_feat)
    ...
    def get_channels(self) -> List[int]:
        # 返回融合后的通道数
        return [self.ufpn.fixed_out_channels] * len(self.fusion_layers)
