...
class RetinaUNetModule(LightningBaseModuleSWA):
    ...
    encoder_cls = Encoder#下采样部分
    decoder_cls = UFPNModular#上采样部分
    ...
    def from_config_plan(cls,
        ...
        encoder = cls._build_encoder(#cls.encoder_cls（即 Encoder）来构建编码器
            plan_arch=plan_arch,
            model_cfg=model_cfg,
            )
    ...
    def _build_encoder(
        ...
        """
        #_build_encoder 是一个类方法,负责根据架构配置plan_arch和模型配置model_cfg来构建编码器。
        ...
        """
        conv = Generator(cls.base_conv_cls, plan_arch["dim"])#conv：通过 Generator 函数生成卷积层，这里使用的是 cls.base_conv_cls（在该类中定义为 ConvInstanceRelu，表示卷积、归一化和 ReLU 激活函数的组合）。
        logger.info(f"Building:: encoder {cls.encoder_cls.__name__}: {model_cfg['encoder_kwargs']} ")
        encoder = cls.encoder_cls(#创建编码器实例
            conv=conv,
            conv_kernels=plan_arch["conv_kernels"],#卷积核大小
            strides=plan_arch["strides"],#卷积层的步长，决定特征图在下采样时的移动步幅。步长越大，特征图下采样的速率越快。
            block_cls=cls.block,#指定用于编码器的基本块（block），即每个编码器阶段的核心模块，通常是一个堆叠的卷积块（如 StackedConvBlock2）。
            in_channels=plan_arch["in_channels"],#输入特征图的通道数
            start_channels=plan_arch["start_channels"],#编码器的起始通道数，这个参数决定了卷积操作中的输出通道数量，并随着网络深度增加。
            stage_kwargs=None,
            max_channels=plan_arch.get("max_channels", 320),#码器的最大通道数，表示编码器网络中最大的特征图通道数量，默认为 320。
